...
class _GameScreenState extends ConsumerState<GameScreen>
  ...
  // ================================================================
  // MIC SENSITIVITY SECTION — tampilkan dB & power realtime
  // ================================================================
  Widget _buildMicSensitivitySection(BalloonGameState gameState) {
    final db = gameState.microphoneDb;
    final power = gameState.breathingPower;
    final dbDisplay = db.isFinite ? '${db.round().clamp(0, 99)} dB' : '-- dB';
    final powerColor = Color(_breathDetectorColor(power));
  ...
  // ================================================================
  // BREATH WAVE — visualisasi power history
  // ================================================================
  Widget _buildBreathWave(BalloonGameState gameState) {
    return Container(
      height: 60,
      decoration: BoxDecoration(
        color: Colors.white.withValues(alpha: 0.3),
      ...
      child: ClipRRect(
        ...
        child: CustomPaint(
          ...
          painter: _BreathWavePainter(
            powerHistory: gameState.powerHistory,
            breathingPower: gameState.breathingPower,
            color: Color(_breathDetectorColor(gameState.breathingPower)),
          ),
        ),
      ),
    );
  ...
  // TIMER DISPLAY
  // ================================================================
  Widget _buildTimerDisplay(BalloonGameState gameState) {
    final isLowTime = gameState.remainingTime <= 10;
...
// ================================================================
// BREATH WAVE PAINTER — visualisasi gelombang napas realtime
// ================================================================

/// CustomPainter yang menggambar wave berdasarkan power history.
///
/// - Sumbu X: waktu (history entries)
/// - Sumbu Y: breathing power (0–100%)
/// - Warna berubah sesuai power terakhir
class _BreathWavePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < powerHistory.length; i++) {
      final x = i * stepX;
      // Y: 0% power di bottom, 100% power di top
      final y = size.height - (powerHistory[i] / 100 * size.height);
  ...
  @override
  bool shouldRepaint(_BreathWavePainter oldDelegate) {
    return oldDelegate.powerHistory != powerHistory ||
        oldDelegate.breathingPower != breathingPower ||
